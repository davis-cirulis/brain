    ...
    switch(action) {
      case 'rotate':
        ...
        return (_this.rotateHandPosition 
          ...
          : (_this.rotateStabilized ? frame.pointables[1].stabilizedTipPosition : frame.pointables[1].tipPosition)
        );
      case 'scale':
        h = _this.hand(frame, 'scale');
        return (_this.scaleHandPosition 
          ? (_this.scaleStabilized ? h.stabilizedPalmPosition : h.palmPosition) 
          : (_this.scaleStabilized ? frame.pointables[1].stabilizedTipPosition : frame.pointables[1].tipPosition)
        );
      case 'pan':
        h = _this.hand(frame, 'pan');
        return (_this.panHandPosition
          ? (_this.panStabilized ? h.stabilizedPalmPosition : h.palmPosition) 
          : (_this.panStabilized ? frame.pointables[1].stabilizedTipPosition : frame.pointables[1].tipPosition)
        );
    };
  };
  ...
  this.rotateObject = function(frame) {
    if (_this.rotateEnabled && _this.applyGesture(frame, 'rotate')) {
	  ...
	  var xDelta = x - _rotateXLast;
	   ...
	   var z = _this.position(frame, 'rotate')[2];
	  if(!_rotateZLast) _rotateZLast = z;
	  var zDelta = z - _rotateZLast;
	  
	 _this.object.rotation.x += xDelta/50
	  _this.object.rotation.y += yDelta/50;
	  _this.object.rotation.z += zDelta/50;
  ...
  this.scaleObject = function(frame) {
    if (_this.scaleEnabled && _this.applyGesture(frame, 'scale')) {
      ...
      if (_this.scaleMin < newScale && newScale < _this.scaleMax) {
		...
		_this.object.scale.x=newScale;
		_this.object.scale.y=newScale;
		_this.object.scale.z=newScale;
      };
